...
func TestBuildEvidenceDocumentsUsesVersionQualifiedIDsAndFileMD5(t *testing.T) {
	docs := BuildEvidenceDocuments("file-md5", 7, "ops", false, []model.EvidenceUnit{{
		...
		Page: 1, Text: "inspection", AssetPath: "merged/file-md5/inspection.png", Image: &model.ImageMetadata{AssetSHA256: "asset"},
	}})
	...
	if docs[0].SourceAsset != "merged/file-md5/inspection.png" {
		t.Fatalf("durable source asset missing: %#v", docs[0])
	}
}
